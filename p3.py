...
def interpret_model(model, vectorizer, test_data_features, test_reviews, y_test, DATA_DIR):
    ...
    Parameters:
    - model: Trained model (RandomForestClassifier or LogisticRegression).
    ...
    # Global Interpretability for RandomForestClassifier and LogisticRegression
    if isinstance(model, RandomForestClassifier) or isinstance(model, LogisticRegression):
    ...
    if isinstance(model, RandomForestClassifier) or isinstance(model, LogisticRegression):
        ...
        print("Model type not supported for interpretability.")
...
def main():
    ...
    submission_forest.to_csv(os.path.join(split_dir, "forest_submission.csv"), index=False)
